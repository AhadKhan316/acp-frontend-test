...
const NextArrow = ({ className, style, onClick }) => (
  <div
    className={`${className} text-green-800 hover:text-green-400 rounded-full transition-all duration-300 sm:w-14 sm:h-14 w-10 h-10 shadow-md hover:shadow-lg`}
    style={{
      ...style,
      display: "flex",
      alignItems: "center",
...
const PyfDelegateCard = ({ speakers, title }) => {
  const settings = {
    infinite: true,
    speed: 600,
    slidesToShow: 4,
              ...
              <div key={index} className="px-3 sm:px-4">
                ...
                >
                  ...
                  <div className="flex-1 p-5 sm:p-6 flex flex-col items-center justify-center text-center bg-gradient-to-b from-white to-gray-50">
                    ...
                    {/* {speaker.role && (
                      <p className="text-sm sm:text-base text-red-700 font-medium truncate">{speaker.role}</p>
                    )}
                    {speaker.bio && (
                  ...
                  {/* Subtle Accent */}
                  <motion.div
                    className="absolute bottom-3 right-3 w-4 h-4 bg-green-800 rounded-full"
